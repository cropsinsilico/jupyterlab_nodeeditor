...
def _dict_conversion(model_file):    
    ...
    # Multiple Models
        ...
        for model in model_file["models"]:
            # Setup dictionary to be filled
            new_dict, new_dict["inputs"], new_dict["outputs"], new_dict['title'] = {}, [], [], model[list(model.keys())[0]]
            
            # Check for singular or multiple in/outputs
            if type(model[list(model.keys())[3]]) is str:
                new_dict["inputs"].append({'title': model[list(model.keys())[3]], 'key': f"temp_in0", 'socket_type': "bytes"})
                new_dict["outputs"].append({'title': model[list(model.keys())[4]], 'key': f"temp_out0", 'socket_type': "bytes"})
            else:
...
def load_model(path):
    ...
    return (model_ins, model_outs, jlne.SocketCollection(socket_types = tuple(socket_list)))
